...
<body class="bg-green-50">
      ...
      <div class="overflow-x-auto">
        <table id="salesTable" class="min-w-full table-auto text-sm text-left">
          ...
          <tbody class="text-gray-700">
            <tr class="border-t">
              <td class="px-4 py-2">Tomatoes</td>
              <td class="px-4 py-2">200 kg</td>
              <td class="px-4 py-2">20,000 BDT</td>
              <td class="px-4 py-2">City Market</td>
            ...
            <tr class="border-t">
              <td class="px-4 py-2">Potatoes</td>
              <td class="px-4 py-2">500 kg</td>
              <td class="px-4 py-2">50,000 BDT</td>
              <td class="px-4 py-2">Retail Chain</td>
            ...
            <tr class="border-t">
              <td class="px-4 py-2">Onions</td>
              <td class="px-4 py-2">300 kg</td>
              <td class="px-4 py-2">30,000 BDT</td>
              <td class="px-4 py-2">Local Grocers</td>
              <td class="px-4 py-2">2025-04-14</td>
              <td class="px-4 py-2 space-x-2">
                <button onclick="openModal('view', this)"
                class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 text-sm">View</button>
              <button onclick="openModal('edit', this)"
  ...
  <script>
      ...
      if (type === "view") {
        ...
        modalContent.innerHTML = `
          <form onsubmit="updateRow(event)">
            <div class="space-y-4">
              <input type="text" id="editProduct" value="${product}" class="w-full px-4 py-2 border rounded-lg" placeholder="Product"/>
              <input type="text" id="editQuantity" value="${quantity}" class="w-full px-4 py-2 border rounded-lg" placeholder="Quantity"/>
              <input type="text" id="editBuyer" value="${buyer}" class="w-full px-4 py-2 border rounded-lg" placeholder="Sold To"/>
    ...
    function openAddModal() {
      ...
      document.getElementById("modalContent").innerHTML = `
        <form onsubmit="addRow(event)">
          <div class="space-y-4">
            <input type="text" id="newProduct" class="w-full px-4 py-2 border rounded-lg" placeholder="Product" required />
            <input type="text" id="newQuantity" class="w-full px-4 py-2 border rounded-lg" placeholder="Quantity" required />
            <input type="text" id="newBuyer" class="w-full px-4 py-2 border rounded-lg" placeholder="Sold To" required />
            <input type="date" id="newDate" class="w-full px-4 py-2 border rounded-lg" required />
    ...
    function updateRow(event) {
      ...
      const buyer = document.getElementById("editBuyer").value;
      const date = document.getElementById("editDate").value;

      const cells = currentRow.getElementsByTagName("td");
      cells[0].innerText = product;
      cells[1].innerText = quantity;
      cells[2].innerText = buyer;
    ...
    function addRow(event) {
      ...
      const buyer = document.getElementById("newBuyer").value;
      ...
      newRow.innerHTML = `
        ...
        <td class="px-4 py-2 space-x-2">
          ...
          <button onclick="openModal('edit', this)" class="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600 text-sm">Edit</button>
        </td>
      `;
